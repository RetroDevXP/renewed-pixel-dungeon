...
public class FireBomb extends Item {
	...
	{
		name = "fire bomb";
		image = ItemSpriteSheet.FIREBOMB;
	...
	protected void onThrow( int cell ) {
		if (Level.pit[cell]) {
			...
			Sample.INSTANCE.play( Assets.SND_BLAST, 1 );
			...
			for (int n : Level.NEIGHBOURS9) {
				...
				if (c >= 0 && c < Level.LENGTH) {
					...
					if (ch != null) {
						int dmg = Random.Int( 1 + Dungeon.depth, 3 + (int)Math.floor(Dungeon.depth * 1.5) ) - Random.Int( ch.dr() );
						if (dmg > 0) {
							ch.damage( dmg, this );
							if (ch.isAlive()) {
								...
								GLog.n( "You killed yourself with a fire bomb..." );
							}
						}
					}
				}
	...
	public String info() {
		return
			"The substances in this bomb causes an explosion along with an ignition of flames. Conveniently, its fuse is lit automatically when the bomb is thrown.";
	}
}
